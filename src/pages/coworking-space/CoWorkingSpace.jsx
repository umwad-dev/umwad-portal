...
const CoWorkingSpacePage = () => {
  ...
  useEffect(() => {
    if (startupDetails?.uid) {
      ...
      const getPartners = async () => {
        const partnersCol = collection(db, "coWorkingSpacePartners");
  ...
  useEffect(() => {
    if (coWorkingSpaces.length > 0) {
      const getStartupInfo = coWorkingSpaces.filter(
      ...
      if (getStartupInfo.length > 0) {
        ...
        return navigate("/coworking-spaces");
      }
    }
  }, [navigate, slug, coWorkingSpaces]);
    ...
    <Fragment>
      ...
      <Container>
        ...
        <ProfileContainer>
          ...
          />
          <InformationContainer>
            <StartupHeader>Co-working Space</StartupHeader>
            <StartupName>
