...
const Modal = ({ data, onCancel, open, title, type }) => {
  ...
  const dDoc = doc(col);
  const imageRef = ref(storage, `programs/${Date.now() + auth.uid}`);
  ...
  const onFinish = async (values) => {
    ...
    if (type === "add") {
      ...
      try {
        ...
        const col = collection(db, "programs");
        const qQuery = query(col, where("slug", "==", values.slug));

        const slugSnapshot = await getDocs(qQuery);
        ...
        openNotification({
          isSuccess: true,
          message: "Successfully added new program data.",
        });
        form.resetFields();
        onCancel();
        setLoading(false);
      ...
      try {
        ...
        const rRef = doc(db, "programs", data.uid);
        ...
        openNotification({
          isSuccess: true,
          message: "Successfully updated selected program data.",
        });
