...
const Products = () => {
  ...
  const col = collection(db, "acceleratorProducts");
  const qQuery = query(col, where("ownerUid", "==", auth.uid));
  ...
  const onConfirm = async (data) => {
    try {
      ...
      const docRef = doc(db, "acceleratorProducts", data.uid);
      ...
      render: (info) => (
        <Space size="middle">
          ...
          <Popconfirm
            ...
            placement="left"
            title="Are you sure you want to delete the selected product?"
          >
  ...
  return (
    <Container>
      ...
      <RowContainer>
        <AddContainer>
          <PrimaryButton name="Add New Product" onClick={onOpenModal} />
        </AddContainer>
      </RowContainer>
      {loading ? (
        <LoadingContainer>
        ...
        <Table columns={columns} data={products ? products : []} rowKey="idx" />
      )}
      <Modal
        data={data}
