...
const FabLabPage = () => {
  ...
  useEffect(() => {
    if (startupDetails?.uid) {
      const getServices = async () => {
        const servicesCol = collection(db, "fabLabServices");
      ...
      const getPartners = async () => {
        const partnersCol = collection(db, "fabLabPartners");
      ...
      const getProducts = async () => {
        const productsCol = collection(db, "fabLabProducts");
  ...
  useEffect(() => {
    if (fabLabs.length > 0) {
      const getStartupInfo = fabLabs.filter((startup) => startup.slug === slug);
      ...
      if (getStartupInfo.length > 0) {
        ...
        return navigate("/fab-labs");
      }
    }
  }, [navigate, slug, fabLabs]);
    ...
    <Fragment>
      ...
      <Container>
        ...
        <ProfileContainer>
          ...
          />
          <InformationContainer>
            <StartupHeader>Fab-Lab</StartupHeader>
            <StartupName>
